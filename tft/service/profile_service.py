...
from tft.utils.riot_api import getAccountFromRiotAPI, getSummonerFromRiotAPI, getSummonerLeagueFromRiotAPI, \
    getLeagueFromRiotAPI, getSummonerMatchListFromRiotAPI, getMatchInfoFromRiotAPI
            ...
            try:
                if not match.objects.filter(match_id=match_id).exists():
                    if server_code.upper() in match_id:
                        with transaction.atomic():
                            match_data = {}
                            json_match_data_response = getMatchInfoFromRiotAPI(match_id)
                            ...
                            for participant in match_info['participants']:
                                ...
                                participant['patch'] = getPatchFromDate(match_datetime)
